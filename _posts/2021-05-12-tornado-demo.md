...
def conn_m():
    conn = pymysql.connect(host='192.168.0.63', port=3306, user='root', passwd='123456', db='t_test')
    cursor = conn.cursor()
    return cursor,conn
...
class JumpHandler(RequestHandler):
    def get(self, *args, **kwargs):
        msg = self.get_cookie("msg")
        self.render("./templates/ppp.html",now_time=(datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")),msg = msg)
        ...
        if (uname and upwd):
            ...
            if userdata:
                if userdata[2] == upwd:
                    ...
                    self.set_cookie(name='msg',value='false',expires=time.time()+60)
...
class PythonHandler(RequestHandler):
    def get(self, *args, **kwargs):
        ...
        if uname:
            self.render("./templates/ttt.html",uname = uname, upwd = upwd)
        else:
            self.set_cookie(name='msg',value='relogin',expires=time.time()+60)
            self.redirect('/index')
...
class RegistHandler(RequestHandler):
    ...
    def post(self, *args, **kwargs):
        ...
        if (uname and upwd):
            ...
            print("r",userdata)
            if not userdata:
                sql = "insert into user_list (uname,upwd) values('{}','{}');".format(uname,upwd)
                cursor.execute(sql)
                conn.commit()
                cursor.close()
                self.set_cookie(name='msg',value='login',expires=time.time()+60)
                self.redirect('/index')
